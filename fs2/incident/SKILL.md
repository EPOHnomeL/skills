...
Write up a production incident as a blameless **post-mortem**, driven by `/grill-with-docs`.
...
2. **Grill only the judgement calls** the thread and code can't settle: status, severity, how much personal ownership to name, and the action items. (This is `/grill-with-docs`.)
